...
public class DotDigraphBuilder {
  ...
  public DotDigraphBuilder withCompoundColor(Map<String, String> color) {
    this.cpdColor = color;
    return this;
  }

  public DotDigraphBuilder withReactionColor(Map<String, String> color) {
    this.rxnColor = color;
    return this;
  }
  ...
  public BinaryGraph<DotNode, DotEdge> build() {
    ...
    for (int i = 0; i < reactionsList.size(); i++) {
      ...
      String rxnEntry = genericReaction.getEntry();
      Double fluxVal = reactionsFluxList.get(i);
      DotNode reactionDotNode = new DotNode();
      reactionDotNode.setColor(rxnColor.get(rxnEntry) == null ? "black" : rxnColor.get(rxnEntry));
      reactionDotNode.setLabel(rxnEntry);
      ...
      for (String cpdEntry : genericReaction.getProductStoichiometry().keySet()) {
        if (!compoundIgnoreList.contains(cpdEntry)) {
          ...
          if (!compoundMap.containsKey(cpdEntry)) {
            ...
            dotNode.setColor(cpdColor.get(cpdEntry) == null ? "black" : cpdColor.get(cpdEntry));
