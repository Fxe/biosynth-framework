...
public class DotGraphFactory {
  ...
  private void constructRxnToCpdEdge(
    ...
    DotNode rxnDotNode = this.reactionDotMap.get(rxnEntry);
    if (cpdDotNode == null) {
      cpdDotNode = new DotNode();
      cpdDotNode.setLabel(cpdEntry + " (not found)");
      logger.warn("{} not found. valid options: {}", cpdEntry, this.metaboliteDotMap.keySet());
    }
    if (rxnDotNode == null) {
      rxnDotNode = new DotNode();
      rxnDotNode.setLabel(rxnEntry + " (not found)");
      logger.warn("{} not found. valid options: {}", rxnEntry, this.reactionDotMap.keySet());
    }
    src = rxnDotNode;
    dst = cpdDotNode;
    DotEdge dotEdge = new DotEdge();
