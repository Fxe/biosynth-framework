...
public class Neo4jMapper {
  ...
  public static OptfluxContainerReactionEntity nodeToModelReaction(Node node) {
    ...
    try {
      ...
      rxn.setDescription(e.getMessage());
    }
    return rxn;
  }
  ...
  public static Map<String, Double> getStoichiometry(Node node, MetabolicModelRelationshipType r) {
    ...
    for (Relationship relationship : node.getRelationships(r)) {
      ...
      Object valObject = relationship.getProperty("stoichiometry", 1.0d);
      double val;
      if (valObject instanceof String) {
        val = Double.parseDouble(valObject.toString());
      } else {
        val = (double) valObject;
      }
