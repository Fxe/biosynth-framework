...
extends AbstractBiosEntityTransform<M, GraphMetaboliteEntity> {
  ...
  protected void configureProperties(GraphMetaboliteEntity centralMetaboliteEntity, M metabolite) {
    ...
    try {
      ...
      for (String key : propertyContainer.keySet()) {
        Object value = propertyContainer.get(key);
        centralMetaboliteEntity.addProperty(key, value);
        logger.trace("annotated property {}:{}", key, value);
      }
  ...
  protected void configureNameLink(GraphMetaboliteEntity gcpd, M cpd) {
    this.configureNameLink(gcpd, cpd.getName());
  }
  protected void configureNameLink(GraphMetaboliteEntity gcpd, String name) {
    if (name != null) {
      this.configureGenericPropertyLink(gcpd, 
                                        name.trim(), 
                                        MetabolitePropertyLabel.Name, 
