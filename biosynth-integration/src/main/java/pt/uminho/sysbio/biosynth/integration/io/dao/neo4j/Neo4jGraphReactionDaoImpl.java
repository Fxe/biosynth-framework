...
implements ReactionHeterogeneousDao<GraphReactionEntity> {
  ...
  private Map<GraphMetaboliteProxyEntity, Map<String, Object>> getReactionMetabolites(
      Node node, ReactionRelationshipType relationshipType) {
    Map<GraphMetaboliteProxyEntity, Map<String, Object>> map = new HashMap<> ();

    logger.debug("Lookup relationship {}", relationshipType);
    for (Relationship relationship : node.getRelationships(relationshipType)) {
      Node other = relationship.getOtherNode(node);
      logger.trace("Found {}:{}", other, Neo4jUtils.getLabels(other));

      Map<String, Object> propertyContainer = 
          Neo4jUtils.getPropertiesMap(relationship);

      long cpdId = other.getId();
      String cpdEntry = (String) other.getProperty("entry", null);
      ...
      entity.setId(cpdId);
      entity.setEntry(cpdEntry);
      entity.setMajorLabel(majorLabel);
      entity.setLabels(Neo4jUtils.getLabelsAsString(other));
      propertyContainer.put("cpdId"   , cpdId);
      propertyContainer.put("cpdEntry", cpdEntry);
  ...
  public GraphReactionEntity getReactionByEntry(String tag, String entry) {
    ...
    logger.debug("Found {} for {}:{}", node, tag, entry);
